...
class _SwitchedCallWidgetState extends State<SwitchedCallWidget> {
  ...
  void initState() {
    super.initState();
    _localRenderer.init(SipConnectFlutter.kLocalVideoCallId, context.read<LogsModel>());
  ...
  List<Widget> _buildVideoControls() {
    ...
    if(widget.myCall.hasVideo) {
      //Received video
      children.add(
        Center(child: SipConnectVideoView(_remoteRenderer)));

      //Camera preview
      children.add(
        SizedBox(width: 130, height: 100, child: SipConnectVideoView(_localRenderer)));
